...
func (c *Compiler) Compile(node parser.Node) error {
	...
	case *parser.IfStmt:
		...
		c.symbol = c.symbol.Fork(true)
		defer func() {
			c.symbol = c.symbol.Parent(false)
		}()
	...
	case *parser.Ident:
		symbol, _, ok := c.symbol.Resolve(node.Name, false)
		if !ok {
	...
	case *parser.FuncLit:
		...
		freeSymbols := c.symbol.FreeSymbols()
		numLocals := c.symbol.MaxSymbols()
	...
	case *parser.ImportExpr:
		...
		if mod := c.modules.Get(node.ModuleName); mod != nil {
			...
			switch v := v.(type) {
			case []byte: // module written in gslang
			...
			if !strings.HasSuffix(moduleName, ".gs") {
				moduleName += ".gs"
			}
...
func (c *Compiler) compileForInStmt(stmt *parser.ForInStmt) error {
	...
	// assign key variable
	if stmt.Key.Name != "_" {
		keySymbol := c.symbol.Define(stmt.Key.Name)
		if itSymbol.Scope == ScopeGlobal {
			c.emit(stmt, parser.OpGetGlobal, itSymbol.Index)
		} else {
			c.emit(stmt, parser.OpGetLocal, itSymbol.Index)
	...
	if stmt.Value.Name != "_" {
		valueSymbol := c.symbol.Define(stmt.Value.Name)
		if itSymbol.Scope == ScopeGlobal {
			c.emit(stmt, parser.OpGetGlobal, itSymbol.Index)
		} else {
			c.emit(stmt, parser.OpGetLocal, itSymbol.Index)
...
func (c *Compiler) compileModule(
	...
	symbol := NewSymbol()
	for _, sym := range c.symbol.BuiltinSymbols() {
		symbol.DefineBuiltin(sym.Index, sym.Name)
	}

	// no global scope for the module
	symbol = symbol.Fork(false)

	// compile module
	moduleCompiler := c.fork(modFile, modulePath, symbol, isFile)
	...
	compiledFunc.NumLocals = symbol.MaxSymbols()
...
func (c *Compiler) fork(
	...
	child := NewCompiler(file, symbol, nil, c.modules, c.trace)
